...
- **`po_catalog.py`** — minimal `.po` reader. `parse_catalog()` → `{msgid: msgstr}` (non-empty only);
  `corpus_chars()` → the unique glyph set used by a locale's translations (non-ASCII by default).
- **`build_lang_font.py`** — builds one subset `.ttf` per locale. Reads the per-profile font manifest
  from `screenshot_gen --dump-locales` (source family + which locales), extracts each locale's corpus,
  and subsets the vendored source TTF (`components/seedsigner/assets/`) with `fontTools`. Output →
  `tools/fontpack/out/<locale>/<locale>.ttf` (+ `manifest.json` with sha256). One `.ttf` serves all
  sizes/resolutions (Tiny TTF rasterizes on demand). CJK ("primary"-chain) subsets **include ASCII** and
  drop GSUB/GPOS/GDEF; Arabic/Thai (Phase 2) must keep layout tables + presentation forms.
- **`gen_localized_scenarios.py`** — translates `tools/scenarios.json` display-text leaves
...
python3 tools/fontpack/build_lang_font.py --locale zh_Hans_CN --locale ja --locale ko
python3 tools/fontpack/gen_localized_scenarios.py            # all catalogs + en
...
`--locale` is repeatable / omittable (defaults to all locales in the manifest / all catalogs).
